...
def main():
    ...
    start_n = time.time()
    ndata_naive = ms.get_neighbour_data_for_all_naive(
        x, y, H, fact=1.0, L=1.0, periodic=True
    )
    stop_n = time.time()
    ...
    for p in range(npart):
        ...
        if nn != ns:
            print(
                "Got different number of neighbours for particle ID",
                ids[p],
                ":",
                nn,
                ns,
            ...
            print(
                "ID part:  {0:6d}  x: {1:14.7f}  y: {2:14.7f}  H: {3:14.7f}".format(
                    ids[p], x[p], y[p], H[p]
                )
            )
            print(
                "ID large: {0:6d}  x: {1:14.7f}  y: {2:14.7f}  r: {3:14.7f}  H: {4:14.7f} r/H: {5:14.7f}".format(
                    idl, xl, yl, rl, Hl, rl / Hl
                )
            )
            print(
                "ID small: {0:6d}  x: {1:14.7f}  y: {2:14.7f}  r: {3:14.7f}  H: {4:14.7f} r/H: {5:14.7f}".format(
                    idS, xS, yS, rS, HS, rS / HS
                )
            )
            quit()
