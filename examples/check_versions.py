...
def main():
    ...
    for row, pcoord in enumerate(pcoords):
        ...
        print(
            "Max difference x:",
            np.max((Aij_Hopkins[:, 0] - Aij_Hopkins_v2[:, 0]) / Aij_Hopkins[:, 0]),
        )
        print(
            "Max difference y:",
            np.max((Aij_Hopkins[:, 1] - Aij_Hopkins_v2[:, 1]) / Aij_Hopkins[:, 1]),
        )
        ...
        print(
            "Max difference x:",
            np.max((Aij_Ivanova[:, 0] - Aij_Ivanova_v2[:, 0]) / Aij_Ivanova[:, 0]),
        )
        print(
            "Max difference y:",
            np.max((Aij_Ivanova[:, 1] - Aij_Ivanova_v2[:, 1]) / Aij_Ivanova[:, 1]),
        )
